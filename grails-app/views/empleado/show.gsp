	...
	<body>
		...
		<div id="show-empleado" class="content scaffold-show" role="main">
			...
			<ol class="property-list empleado">
				...
				<g:if test="${empleadoInstance?.serviciosRealizados}">
				<li class="fieldcontain">
					<span id="serviciosRealizados-label" class="property-label"><g:message code="empleado.serviciosRealizados.label" default="Servicios Realizados" /></span>
					
						<g:each in="${empleadoInstance.serviciosRealizados}" var="s">
						<span class="property-value" aria-labelledby="serviciosRealizados-label"><g:link controller="servicio" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
